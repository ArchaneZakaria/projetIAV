...
public function uploadFile($data = ''){
  ...
  $this->display($data);
   $this->template->set_partial('container', 'upload_view', array());
  ...
  break;
   ...
   //modifier prologation
    ...
    //modifier prologation
          ...
          $etat_delete = $this->db->delete('etablissement', $conditions);*/
        $datenotifv = date("Y-m-d", strtotime($departr ."+". $nb * 2 ."years" . "+". $nbM ."Month" ."-". 1 ."years" ));
        $departrv = date("Y-m-d", strtotime($departr ."+". $nb * 2 ."years" . "+". $nbM ."Month"));
// /  echo $nb. ' ' .$nb_pr_mx;die;
        if($nb_pr_mx >= $nb){
                ...
                <div class="form-group">
                    <div class="col-md-12">
                        ...
                        <div class="btn btn-danger btn-file">
                            <i class="glyphicon glyphicon-folder-open"></i> &nbsp;Telecharger Fichier <input type="file"  id="file-simple">
                        </div>
        </span>
                    </div>
                </div>
...
public function do_upload()
       {
               ...
               $config['allowed_types']        = 'docx|pdf|gif|jpg|png';
               $config['max_size']             = 100000;
               $config['max_width']            = 102400;
               $config['max_height']           = 768000;
               // $new_name = time().$_FILES["file-simple"]['name'];
               // $config['file-simple'] = $new_name;
