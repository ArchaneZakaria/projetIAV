...
class Ctr_reparation extends Master
{
  ...
  public function reparation($data = '',$id = null){
     ...
     public function validerReparation()
     {
       ...
       $result = $this->reparation->update(array('status_reparation' => 'VP'),array('id_reparation' => $id_rep),'udate_reparation');
       if($result){
     ...
     public function exectution_final_reparation()
     {
       ...
       $result = $this->reparation->update(array('status_reparation'=>$status),array('id_reparation' => $id),'udate_reparation');
      if($result){
        echo $etat;
      }
     ...
     public function getformExecte(){
        $prestataire = $this->prestataire->read("*");
        $id    = $this->input->post('id');
        $reparation      =  $this->reparation->read("*",array('id_reparation' => $id));
        $type_reparation = $reparation[0]->id_type_reparation;
        if($type_reparation == 1){
          echo $this->template->load_view('reparation/formExecte_view.php',array('prestataire'=>$prestataire,'id'=>$id));
        }else if($type_reparation == 2) {
     ...
     public function execute_reparation_2()
     {
      ...
      if($this->form_validation->run()){
        ...
        $reparation      =  $this->reparation->read("*",array('id_reparation' => $id));
        $type_reparation = $reparation[0]->id_type_reparation;
        if($type_reparation == 2){

          $Idtypebudget_Transport =  $this->typeBudget->read("*",array('libelle_type_budget' => 'REPARATION '))[0]->id_type_budget;
  ...
  {
      $id              = $this->input->post('id');
      $id_reparat_arcl = $this->article->read("iav_reparation_id_reparation",array('id_article_reparation' => $id))[0]->iav_reparation_id_reparation;
