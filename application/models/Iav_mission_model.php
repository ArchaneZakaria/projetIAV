...
$queryDetail = $this->db->query("SELECT * FROM iav_mission inner join iav_personnel_mission on (iav_mission.id_mission = iav_personnel_mission.iav_mission_id_mission) inner join iav_personel on (iav_mission.cby_mission = iav_personel.id_personel) inner join iav_mission_imputes on ( iav_mission_imputes.id_mission_imputes = iav_mission.id_mission_imputes) where iav_mission.etat_mission ='E' and iav_mission.id_mission ='" . $idMiss."' and iav_mission.deleted_mission='N'");
...
return $this->db->insert_id();
    ...
    public function GetMissionsByVeichule($idvcl = null)
    {
      ...
      mission.date_debut,mission.date_fin,mission.id_mission,chfr.id_chauffeur,mission.numero_ordre_mission
      ...
      mdl.id_model               = vcl.id_model and mission.deleted_mission = 'N' and YEAR(NOW()) = YEAR(cdate_mission) ";
      if(isset($idvcl)){
...
*/
       ...
       public function GetMissonsByChauffeur($idchfr)
    {
      ...
      vcl.matricule_vehicule,mission.date_debut,mission.date_fin,mission.id_vehicule from iav_mission as mission ,iav_vehicule as vcl, iav_chauffeur as chf
       where mission.id_chauffeur = chf.id_chauffeur and mission.id_vehicule = vcl.id_vehicule  and mission.etat_mission = 'V' and mission.deleted_mission='N' ";
       if($idchfr != null){
   ...
   public function getLastMissionsPageHome()
   {
     ...
     $sql ="select mission.id_mission,mission.libeller_mission,mission.date_debut,pers.nom_personel,pers.prenom_personel,chf.prenom_chauffeur,
      ...
      depart.id_departement= ptd.id_departement AND mission.deleted_mission = 'N' and YEAR(mission.cdate_mission) = '".$anne_valider."' and mission.etat_mission='V'";
      $query            = $this->db->query($sql);
      $tab_all_missions =  $query->result();
      $tab_temp = array();
	  foreach($tab_all_missions as $key => $value){
		  if(!isset($tab_temp[$value->id_mission])){
			  $tab_temp[$value->id_mission] = $value;
		  }
	  }
	  return $tab_temp ;
	  //echo '<pre>';print_r($tab_temp);die;
      //return array_slice($tab_temp,count($tab_temp));
   }
