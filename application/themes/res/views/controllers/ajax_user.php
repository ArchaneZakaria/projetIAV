...
class Ajax_user extends Master_user
{
    ...
    public function __construct()
    {
		...
		$this->load->model('Doctorants_model', 'doctorant');
		$this->load->model('Presse_model', 'presse');
        $this->load->model('job_model', 'job');
        $this->load->model('candidatureetudiant_model', 'candidatureetudiantb');
        $this->load->model('document_model', 'document');
        $this->load->model('distinction_model', 'distinction');
    }
...
/*$user = $this->user->read('id,userLogin', array(
    ...
    public function login($type = USER_TYPE_ADMIN)
    {
				...
				case USER_TYPE_NAKRO:
                    ...
                    if (!empty($user)) {
                        ...
                        $this->set_session_token(array(
                            'user_id' => $user->id,
                            'user_login' => $user->userLogin,
                            'user_type' => USER_TYPE_NAKRO));
                        echo json_encode(array(
                            'status' => '1',
                            'location' => 'nakro/professors',
                            'message' => 'bienvenue <strong>' . $user->userLogin . '</strong> !'));
                    }
                    break;
        ...
        if($user->userLogin == 'rhadmin'){
                        echo json_encode(array(
                            'status' => '1',
                            'location' => 'admin/rh',
                            'message' => 'bienvenue <strong>' . $user->userLogin . '</strong> !'));
                    }else if($user->userLogin == 'admin'){
                ...
                case USER_TYPE_DOCT:
                  /*  $this->load->model('Doctorants_model', 'doctorant');
                    $user = $this->doctorant->read('id,doctLogin', array(
                        'doctLogin' => $this->input->post('login'),
                        'doctPassword' => sha1($this->input->post('password')),
                        'doctIsActive' => '1'));

                    if (!empty($user)) {
                        $user = $user[0];
                        $this->doctorants->set_last_login($user->id);
                        $this->set_session_token(array(
                            'user_id' => $user->id,
                            'user_login' => $user->doctLogin,
                            'user_type' => USER_TYPE_DOCT));
							*/
                        echo json_encode(array(
                            'status' => '1',
                            'location' => 'doctorant/index',
                            'message' => 'bienvenue <strong>' . 'rr' . '</strong> !'));
                    /*}*/
                    break;
...
public function numbernewss(){
$optionSelect = $this->input->post('optionSelect');
echo json_encode(array(
                            'status' => '1',
                            'location' => 'newsv/indexn/' . $optionSelect,
                            'message' => 'bienvenue <strong>' . $optionSelect . '</strong> !'));
}
public function searchnewss(){

$searchvalue = $this->input->post('searchvalue');

echo json_encode(array(
                            'status' => '1',
                            'location' => 'newsv/index/' . $searchvalue,
                            'message' => 'bienvenue <strong>' . $searchvalue . '</strong> !'));
     }

	/** add update nakro */

	  public function op_professor_nakro()
    {
        if (!$this->user_is_connected(USER_TYPE_NAKRO))
            $this->set_public_message("Vous n'êtes pas connecté, merci de se connecter.",
                'info', 'connexion/nakro');
...
//                'branch' => $branch,
	...
	public function op_presse()
    {
        ...
        if ($this->form_validation->run()) {
          /*  $this->form_validation->set_rules('permanent_link', 'Lien permanent',
                'is_unique[presses.presseLinkPermanent]');*/
            $this->form_validation->set_message('is_unique',
                'Le champ <strong>%s</strong> est une clé unique, un presse existe déjà avec le lien: <i>' .
                $this->input->post('permanent_link') . '</i>.');
            $permanent_link = convert_accented_characters(strtolower($this->input->post('permanent_link')));
            ...
            if (($id = $this->input->post('id'))) {
                $message = "Le presse a été modifié avec succès.";

				$result=array(
                    'title' => $this->input->post('title'),
                    'description' => $this->input->post('description'),
                    'descriptionBrut' => trim_str($this->input->post('description')),
                    'presseLinkPermanent' => $permanent_link,
                    //'type' =>$typeetd,
                    'user_id' => $this->user_id);



				   $resulte = $this->presse->update($result, $id);

            } else {
                if ($this->form_validation->run()) {
                    $message = "Le presse a été ajouté avec succès.";
					$result =array(
                        'title' => $this->input->post('title'),
                        'description' => $this->input->post('description'),
                        'descriptionBrut' => trim_str($this->input->post('description')),
					...
					//$this->load->model('Presse_model','presse');
				    $resulte = $this->presse->create($result);
    ...
    public function delete_elm($type = '')
    {
        ...
        switch ($type) {
            ...
            case 'professors':
                ...
                break;
			 case 'professorsnakr':
                $del = $this->user_is_connected(USER_TYPE_NAKRO) ? true : false;
                $model = $this->professor;
                break;
            case 'pub':
            case 'publication':
    ...
    public function op_news()
    {
        ...
        $this->form_validation->set_rules($prefix . '_description', 'Description',
            'required|trim');
		 $this->form_validation->set_rules($prefix . '_campus', 'Campus',
        'required|trim');
         $this->form_validation->set_rules($prefix . '_actuapesa', 'Actualtées',
        'required|trim');
            ...
            if ($errors === false) {
                if (($news_id = $this->input->post($prefix . '_id'))) {
                    ...
                    $options = array(
						...
						'id_campus' => $this->input->post($prefix . '_campus'),
                        'typeApesa' => $this->input->post($prefix . '_actuapesa'),
    ...
    /*** recrutement et appel à candidature **/


   public function op_distinction(){
if (!$this->user_is_connected(USER_TYPE_ADMIN))
            $this->set_public_message("Vous n'êtes pas connecté, merci de se connecter.",
                'info', USER_TYPE_ADMIN_HOME);
        $prefix = 'distinction';
  ...
  if (($distinction_id = $this->input->post($prefix . '_id'))) {
                    $message = "La distinction a été modifié avec succès.";
                    $options = array(
                        'title' => htmlspecialchars($this->input->post($prefix . '_title')),
                        'description' => $this->input->post($prefix . '_description'),
                        'descriptionBrut' => trim_str($this->input->post($prefix . '_description')),
                        /*'id_campus' => $this->input->post($prefix . '_campus'),
                        'typeApesa' => $this->input->post($prefix . '_actuapesa'),*/
                        //                            'newsPermanentLink' => $link,
                       /* 'newsVideo' => $youtube_link,*/
                        'disctinctionIsPublished' => $this->input->post($prefix . '_is_published') == '1' ? '1' :
                            '0',
                        'user_id' => $this->user_id);
                   /* if (isset($file_image_data['file_name']))
                        $options['newsImg'] = $file_image_data['file_name'];*/
                    $result = $this->distinction->update($options, $distinction_id);
                    echo json_encode(array('status' => '1', 'url' => 'administration/distinction'));
                } else {

            $message = "La distinction a été ajouté avec succès.";
                    $options = array(
                        'title' => htmlspecialchars($this->input->post($prefix . '_title')),
                        'description' => $this->input->post($prefix . '_description'),
                        /*'id_campus' => $this->input->post($prefix . '_campus'),*/
                    ...
                    /*if (isset($file_image_data['file_name']))
                        $options['newsImg'] = $file_image_data['file_name'];*/
                    $result = $this->distinction->create($options);
  ...
  if (($candidature_id = $this->input->post($prefix . '_id'))) {
                    $message = "L'actualité a été modifié avec succès.";
                    $options = array(
                        'title' => htmlspecialchars($this->input->post($prefix . '_title')),
                        'description' => $this->input->post($prefix . '_description'),
                        ...
                        //                            'newsPermanentLink' => $link,
                       /* 'newsVideo' => $youtube_link,*/
                        'etudiantIsPublished' => $this->input->post($prefix . '_is_published') == '1' ? '1' :
                            '0',
                        'user_id' => $this->user_id);
                   /* if (isset($file_image_data['file_name']))
                        $options['newsImg'] = $file_image_data['file_name'];*/
                    $result = $this->candidatureetudiantb->update($options, $candidature_id);
                } else {
                    ...
                    /*if (isset($file_image_data['file_name']))
                        $options['newsImg'] = $file_image_data['file_name'];*/
                    $result = $this->candidatureetudiantb->create($options);
                }
            ...
            if ($errors === false) {
                ...
                if ($result) {
                    $this->set_public_message($message, 'success');
                    echo json_encode(array('status' => '1', 'url' => 'administration/candidatureEtudiant'));
                } else
                    $errors = "Une erreur est survenue lors du traitement, merci de réessayer.";
            }
        } else
  ...
  if (($job_id = $this->input->post($prefix . '_id'))) {
                    $message = "L'actualité a été modifié avec succès.";
                    $options = array(
                        'title' => htmlspecialchars($this->input->post($prefix . '_title')),
                        'description' => $this->input->post($prefix . '_description'),
                        ...
                        //                            'newsPermanentLink' => $link,
                       /* 'newsVideo' => $youtube_link,*/
                        'jobIsPublished' => $this->input->post($prefix . '_is_published') == '1' ? '1' :
                            '0',
                        'user_id' => $this->user_id);
                   /* if (isset($file_image_data['file_name']))
                        $options['newsImg'] = $file_image_data['file_name'];*/
                    $result = $this->job->update($options, $job_id);
                } else {
                    $message = "L'offre a été ajouté avec succès.";
                    $options = array(
                        'title' => htmlspecialchars($this->input->post($prefix . '_title')),
                    ...
                    /*if (isset($file_image_data['file_name']))
                        $options['newsImg'] = $file_image_data['file_name'];*/
                    $result = $this->job->create($options);
                }
            ...
            if ($errors === false) {
                ...
                if ($result) {
                    $this->set_public_message($message, 'success');
                    echo json_encode(array('status' => '1', 'url' => 'administration/job'));
                } else
                    $errors = "Une erreur est survenue lors du traitement, merci de réessayer.";
            }
        } else
