...
public function mission($data = '',$id='',$idEnti='')
  ...
  /* $this->set_breadcrumb(array("" => ''));*/
   switch($data){
     ...
     $this->template->set_partial('container', 'add_ordre_mission_view', array('data' => $data,'desn'=>$desn,'administratif'=>$administratifs,'ensg'=>$ensg,'departm'=>$departmt,'iDfiliereD' => $iDfiliereD));
      ...
      case 'modifier' :
            ...
            if (($id = intval($id)) && !empty($id) && ($mission = $this->mission->read("*",array('id_mission' => $id)))) {
                 ...
                 $this->template->set_partial('container', 'add_ordre_mission_view', array('data' => $data,'desn'=>$desn,'mission' => (array) $mission[0],'hidden' => $hidden,'op_btn_value' => 'Modifier','administratif'=>$administratifs,'ensg'=>$ensg,'departm'=>$departmt));
                 }else{
