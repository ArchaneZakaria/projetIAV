<?php defined('BASEPATH') or exit('No direct script access allowed');

class Role_model extends MY_Model {

    function __construct() {
        parent::__construct();
        $this->table = 'role';
        $this->table_id = 'idrole';
    }

 public function ajouter_role($title,$description)
    {
       //$this->db->set('newsTitle',$title);
//       $this->db->set('newsDescription',$description);
//       return $this->db->insert($this->table);

return null;

    }

    /**
     *  ?ite une news d??existante
     */
    public function editer_role($id,$title,$description)
    {

    }

    /**
     *  Supprime une news
     */
    public function supprimer_role($id)
    {
        return $this->db->where('id',id)
                         ->delete($this->table);
    }

    /**
     *  Retourne une liste de news
     */
 public function list_role($nb,$debut){
     return   $this->db->select("*")
                  ->from($this->table)
                   ->limit($nb,$debut)
                   ->order_by('idrole','desc')
                   ->get()
                   ->result();


    }
/*
    public function get_search ($match) {
        if (!isset($match) || empty($match))
            return array();
        $this->db->where('newsIsPublished', '1');
        $this->db->like('newsTitle', $match);
        $this->db->or_like('newsDescriptionBrut',$match);
        return $this->db->get($this->table)->result();
    }


	public function get_archive ($datearchive) {
        if (!isset($datearchive) || empty($datearchive))
            return array();

        $this->db->where('newsIsPublished', '1');
		//$match='2016-01-%';
		$this->db->like('newsCreatedOn',"$datearchive");
        //$this->db->like('newsCreatedOn','%2016-04-14%' );
        return $this->db->get($this->table)->result();
    }
*/
}
